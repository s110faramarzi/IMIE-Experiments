...
////////////////////////////////////////////////////////////////////////////////
   ...
   int pxl;
    //struct threeNum num;
   FILE *fptr;
   if ((fptr = fopen("wm100720.bin","rb")) == NULL){
...
////////////////////////////////////////
    double CF = 6.0;  //center frequency
    int SR = 100;    //Fs
    double w; 
    ...
    double phase[nf][5];
    double prc[nf][5];
    double prcCoeff[5]={0.01,-0.0581,-0.1,-0.02,0.02};
    char fileName[] = "120-1";

////////////////////////////////////////
//////////////////// control parameters. See Matchen & Moehlis 2018 paper on J. Comp. Neurosci. 
////////////////////////////////////////

   double  p=1.5;
   double  alpha=0.1;
   double phasediff[nf][4];
   double gfunc[nf][4];
   double a[nf][4];
   double r[nf][4];   
   double tmparr[4]; //to sort the phases at each time
////////////////////////////////////////
//////////////////// stim
////////////////////////////////////////
    

    stim[0]=0.0; //later is used for returning stim to background value


    //int N=sizeof(my_array)/sizeof(my_array[0]);
    double complex Xjw[nf][5]={0.0+0.0*I};
    w= 2*pi*CF/SR;
    tau = 4*SR/CF;
...
/////////////////////////////// prc function ///////////////////////////////
double p1=-9.8705*(pow(1,-12.0))/M_PI;
double p2=3.9125*(pow(1.0,-11.0))/M_PI;
double p3=-4.6*(pow(1.0,-9.0))/M_PI;
double p4=-9.4*(pow(1.0,-9.0))/M_PI;
double p5=5.1*(pow(1.0,-7.0))/M_PI;
double p6=-1.5*(pow(1.0,-6.0))/M_PI;
double p7=-2.2*(pow(1.0,-5.0))/M_PI;
double p8=9.1*(pow(1.0,-5.0))/M_PI;
double p9=5.44*(pow(1.0,-4.0))/M_PI;
double p10= -0.0019*(pow(1.0,0.0))/M_PI; 
double p11=-0.006*(pow(1.0,0.0))/M_PI; 
double p12=0.011*(pow(1.0,0.0))/M_PI;
double p13=6.0*(pow(1.0,-5.0))/M_PI;


double prcFunc(double phse) 
{ 
    
    return 1*(p1*(pow(phse,12.0))+p2*(pow(phse,11.0))+p3*(pow(phse,10.0))+p4*(pow(phse,9.0))+p5*(pow(phse,8.0))+p6*(pow(phse,7.0))+p7*(pow(phse,6.0))+p8*(pow(phse,5.0))+p9*(pow(phse,4.0))+p10*(pow(phse,3.0))+p11*(pow(phse,2.0))+p12*(pow(phse,1.0))+p13); 
    //return (phse*(-0.1)); 
...
////////////////////////////////////////////////////////////////////////////
    ...
    /*
     * set size variables for image and initial subimagefileName
     */
     ...
     */
    char buffer[32];
	snprintf(buffer, sizeof(char) * 32, "TimeComp_%s.bin",fileName);  ///time component
	FILE *file = fopen(buffer, "wb");
    char bufferdiff[32];
	snprintf(bufferdiff, sizeof(char) * 32, "TimeCompDiff_%s.bin",fileName);  ///time component
	FILE *filediff = fopen(bufferdiff, "wb");
    char buffer2[32];
	snprintf(buffer2, sizeof(char) * 32, "Xjw_%s.bin",fileName);  ///xjw
        FILE *flep;
        flep=fopen(buffer2, "wb");   // Xjw values
    char buffer3[32];
	snprintf(buffer3, sizeof(char) * 32, "stim_%s.bin",fileName);  ///stim
        FILE *stimData;
        stimData=fopen(buffer3, "wb");   // stim data
    char buffer4[32];
	snprintf(buffer4, sizeof(char) * 32, "phase_%s.bin",fileName);  ///phase
        FILE *phsData;
        phsData=fopen(buffer4, "wb");   // phase data
    char buffer5[32];
	snprintf(buffer5, sizeof(char) * 32, "prc_%s.bin",fileName);  ///prc
        FILE *prcData;
        prcData=fopen(buffer5, "wb");   // phase data
    char buffer6[32];
	snprintf(buffer6, sizeof(char) * 32, "frames_%s.bin",fileName);  ///raw frames
        FILE   *tmpf; 
    ...
    for (lc = 0; lc < loops && !GetOut; lc++)
    {
		...
		//for(rows=0; rows<s_height; rows++) DataArray[rows]=(u_char *) malloc(s_width*sizeof(u_char));
		    ...
		    //fprintf(file," ");
                    ...
                    if (aa>=1){
                    Xjw[aa][0] = D*Xjw[aa-1][0]+timecomp[aa][0]-timecomp[aa-1][0];   //all pixels
                    //fwrite(Xjw, sizeof(Xjw), 1, flep);
                    //fwrite(flep,"%g+%gi\n",creal(Xjw[aa]),cimag(Xjw[aa]));
                   //fwrite(creal(Xjw[aa]),cimag(Xjw[aa]),imagesize, 1,flep);
                    phase[aa][0]=carg(Xjw[aa][0])+M_PI;
                    //printf("%f\n",phase[aa]);
                    
                   //////////////////////////////////////// calculate the phase and PRC of 4 regions ////////////////////////////////////////////////////////////////
                   
                    
                    Xjw[aa][1] = D*Xjw[aa-1][1]+timecomp[aa][1]-timecomp[aa-1][1]; //cluster 1
                    phase[aa][1]=carg(Xjw[aa][1])+M_PI;
                    //double mm = prcFunc(phase[aa][1]); 
  
                   // printf("m is %f\n", prcFunc(phase[aa][1])); 
                    //printf("%f\n",phase[aa][1]);
                    
                    Xjw[aa][2] = D*Xjw[aa-1][2]+timecomp[aa][2]-timecomp[aa-1][2];//cluster 2
                    phase[aa][2]=carg(Xjw[aa][2])+M_PI;
                    
                    Xjw[aa][3] = D*Xjw[aa-1][3]+timecomp[aa][3]-timecomp[aa-1][3];//cluster 3
                    phase[aa][3]=carg(Xjw[aa][3])+M_PI;
                   
                    Xjw[aa][4] = D*Xjw[aa-1][4]+timecomp[aa][4]-timecomp[aa-1][4];//cluster 4
                    phase[aa][4]=carg(Xjw[aa][4])+M_PI;
		  ...
		  //      printf("%d ||%f\n",aa,tmparr[ir]);
		   // }
                    ///phase difference between cluster 1 and 2
                    phasediff[aa][0]=(tmparr[1]-tmparr[0]);
                   // printf("%f\n",phasediff[aa][0]);
                    if (phasediff[aa][0]>=0 && phasediff[aa][0]<=(M_PI/2.0)){
                    gfunc[aa][0]=2*(phasediff[aa][0]);
                    r[aa][0]=(1.0/(pow(2*phasediff[aa][0],p)))-(1.0/pow((M_PI),p));
                    a[aa][0]=p*(prcFunc(tmparr[1])-prcFunc(tmparr[0]))/((pow(2.0,p))*(pow(phasediff[aa][0],(p+1))));
                    }
                    else {
                    gfunc[aa][0]=2*(phasediff[aa][0]-(M_PI/2.0))/3 + (M_PI/1.0);
                    r[aa][0]=1.0/(pow(((M_PI/1.0)-2.0*(phasediff[aa][0]-(M_PI/2.0))/3.0),p))-(1.0/pow((M_PI/1.0),p));
                    a[aa][0]=-2.0*p*(prcFunc(tmparr[1])-prcFunc(tmparr[0]))/(3.0*(pow((M_PI/1.0)-((2.0/3.0)*(phasediff[aa][0]-(M_PI/2.0))),(p+1))));
                    }
                    ///phase difference between cluster 2 and 3
                    phasediff[aa][1]=(tmparr[2]-tmparr[1]);
                    if (phasediff[aa][1]>=0 && phasediff[aa][1]<=(M_PI/2.0)){
                    gfunc[aa][1]=2*(phasediff[aa][1]);
                    r[aa][1]=(1.0/(pow(2*phasediff[aa][1],p)))-(1.0/pow((M_PI/1.0),p));
                    a[aa][1]=p*(prcFunc(tmparr[2])-prcFunc(tmparr[1]))/((pow(2.0,p))*(pow(phasediff[aa][1],(p+1))));
                    }
                    else {
                    gfunc[aa][1]=2*(phasediff[aa][1]-(M_PI/2.0))/3 + (M_PI/1.0);
                    r[aa][1]=1.0/(pow(((M_PI/1.0)-2.0*(phasediff[aa][1]-(M_PI/2.0))/3.0),p))-(1.0/pow((M_PI/1.0),p));
                    a[aa][1]=-2.0*p*(prcFunc(tmparr[2])-prcFunc(tmparr[1]))/(3.0*(pow((M_PI/1.0)-((2.0/3.0)*(phasediff[aa][1]-(M_PI/2.0))),(p+1))));
                    }
                    ///phase difference between cluster 3 and 4
                    phasediff[aa][2]=(tmparr[3]-tmparr[2]);
                    if (phasediff[aa][2]>=0 && phasediff[aa][2]<=(M_PI/2.0)){
                    gfunc[aa][2]=2*(phasediff[aa][2]);
                    r[aa][2]=(1.0/(pow(2*phasediff[aa][2],p)))-(1.0/pow((M_PI/1.0),p));
                    a[aa][2]=p*(prcFunc(tmparr[3])-prcFunc(tmparr[2]))/((pow(2.0,p))*(pow(phasediff[aa][2],(p+1))));
                    }
                    else {
                    gfunc[aa][2]=2*(phasediff[aa][2]-(M_PI/2.0))/3 + (M_PI/1.0);
                    r[aa][2]=1.0/(pow(((M_PI/1.0)-2.0*(phasediff[aa][2]-(M_PI/2.0))/3.0),p))-(1.0/pow((M_PI/1.0),p));
                    a[aa][2]=-2.0*p*(prcFunc(tmparr[3])-prcFunc(tmparr[2]))/(3.0*(pow((M_PI/1.0)-((2.0/3.0)*(phasediff[aa][2]-(M_PI/2.0))),(p+1))));
                    }
                    ///phase difference between cluster 4 and 1
                    phasediff[aa][3]=(tmparr[0]-tmparr[3]+(M_PI*2.0));
                    //printf("%f %f %f\n",tmparr[0],tmparr[3],phasediff[aa][3]);
                    //if (phasediff[aa][3]<0.000000){phasediff[aa][3]=0.0;
			//}
                    if (phasediff[aa][3]>=0 && phasediff[aa][3]<=(M_PI/2.0)){
                    gfunc[aa][3]=2*(phasediff[aa][3]);
                    r[aa][3]=(1.0/(pow(2*phasediff[aa][3],p)))-(1.0/pow((M_PI/1.0),p));
                    a[aa][3]=p*(prcFunc(tmparr[0])-prcFunc(tmparr[3]))/((pow(2.0,p))*(pow(phasediff[aa][3],(p+1))));
                    }
                    else {
                    gfunc[aa][3]=2*(phasediff[aa][3]-(M_PI/2.0))/3 + (M_PI/1.0);
                    r[aa][3]=1.0/(pow(((M_PI/1.0)-2.0*(phasediff[aa][3]-(M_PI/2.0))/3.0),p))-(1.0/pow((M_PI/1.0),p));
                    a[aa][3]=-2.0*p*(prcFunc(tmparr[0])-prcFunc(tmparr[3]))/(3.0*(pow((M_PI/1.0)-((2.0/3.0)*(phasediff[aa][3]-(M_PI/2.0))),(p+1))));
                    }
       ...
       if (aa>1500 && aa<3000){ 
        stim[aa]=0.002*(sin(a[aa][0]+a[aa][1]+a[aa][2]+a[aa][3])*(pow((alpha*(r[aa][0]+r[aa][1]+r[aa][2]+r[aa][3])),0.5)));
        //stim[aa]=sin((M_PI/180.0)*90.0);
       //printf("%f\n",M_PI);
       }
       else
       {
